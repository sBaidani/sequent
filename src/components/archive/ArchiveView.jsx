  ...
  return (
    <>
      <div class="h-[60px] min-h-[60px] border-b border-border flex items-center justify-between px-6 bg-body/40 backdrop-blur-md sticky top-0 z-50">
        <div class="font-display lowercase text-xl font-bold text-primary tracking-wide">Archive</div>
      </div>

      <div class="overflow-y-auto p-6 flex flex-col gap-6 max-w-[800px] mx-auto w-full">
        
        <div class="flex gap-2 bg-primary/5 p-1 rounded-lg self-start">
          <button 
            onClick={() => setFilter('all')}
            class={`px-3 py-1.5 rounded-md border-none font-semibold cursor-pointer transition-colors ${filter() === 'all' ? 'bg-primary/15 text-primary' : 'bg-transparent text-secondary hover:text-primary'}`}
          >All</button>
          <button 
            onClick={() => setFilter('events')}
            class={`px-3 py-1.5 rounded-md border-none font-semibold cursor-pointer transition-colors ${filter() === 'events' ? 'bg-primary/15 text-primary' : 'bg-transparent text-secondary hover:text-primary'}`}
          >Events</button>
          <button 
            onClick={() => setFilter('tasks')}
            class={`px-3 py-1.5 rounded-md border-none font-semibold cursor-pointer transition-colors ${filter() === 'tasks' ? 'bg-primary/15 text-primary' : 'bg-transparent text-secondary hover:text-primary'}`}
          >Tasks</button>
        </div>
          ...
          <For each={groupedArchive()}>
            {(group) => (
              <div class="flex flex-col gap-3">
                <div class="text-lg font-extrabold text-primary">{group.header}</div>
                    ...
                    {(item) => (
                      <div class="bg-card border border-border rounded-xl p-3.5 flex items-center justify-between transition-all opacity-70">
                        <div class="flex flex-col gap-1">
                          <div class="text-[15px] font-bold text-primary/90">{item.title}</div>
                          <div class="text-xs font-semibold text-disabled flex items-center gap-1.5">
                            {item._type === 'event' ? (
                              <span>Event • {(() => {
