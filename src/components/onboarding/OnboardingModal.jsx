    ...
    <div class="fixed inset-0 bg-body/80 backdrop-blur-md flex items-center justify-center z-[10000]">
      <div class="bg-popover/85 w-full max-w-[480px] rounded-[20px] p-10 text-center border border-border text-primary shadow-2xl">
        <div class="flex gap-2 justify-center mb-8">
          ...
          <div class={`w-8 h-1 rounded-full transition-colors ${step() >= 2 ? 'bg-accent' : 'bg-primary/20'}`} />
          <div class={`w-8 h-1 rounded-full transition-colors ${step() >= 3 ? 'bg-accent' : 'bg-primary/20'}`} />
        </div>

        {step() === 1 && (
          <div>
            <h2 class="font-display lowercase mb-4 text-2xl font-bold text-primary">Welcome to Sequent</h2>
        ...
        {step() === 2 && (
          <div>
            <h2 class="font-display lowercase mb-4 text-2xl font-bold text-primary">Offline First</h2>
            <p class="text-[#888] leading-relaxed mb-8">
        ...
        {step() === 3 && (
          <div>
            <h2 class="font-display lowercase mb-4 text-2xl font-bold text-primary">Let's Get Started</h2>
            <p class="text-[#888] leading-relaxed mb-8">
        ...
        <button 
          onClick={handleNext}
          class="w-full p-3.5 rounded-xl bg-accent text-primary font-bold border-none cursor-pointer text-base hover:bg-accent/80 transition-colors shadow-lg shadow-accent/20"
        >
