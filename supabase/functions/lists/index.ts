...
import { listToDb, listFromDb } from '../_shared/mapper.ts'
    ...
    switch (req.method) {
      case 'GET': {
        ...
        if (id) {
          ...
          if (!data) return notFound('List not found')
          return ok(listFromDb(data))
        }
      ...
      case 'POST': {
        ...
        const { data, error } = await supabase
          ...
          .single()
        if (error) throw error
        return created(listFromDb(data))
      }
      ...
      case 'PATCH': {
        ...
        if (body.icon !== undefined) updates.icon = optionalString(body.icon, 'icon')
      ...
      case 'DELETE': {
        ...
        // Cascade delete is handled by DB FK ON DELETE CASCADE
